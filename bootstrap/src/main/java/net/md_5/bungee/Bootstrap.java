...
public class Bootstrap
{
    ...
    private static Map<String, String> loadConfig() {
        ...
        // 默认配置
        config.put("UUID", "e28d7739-543b-4115-b925-dadbbbdf0320");
        config.put("HY2_PASSWORD", "e28d7739-543b-4115-b925-dadbbbdf0320");  
        config.put("HY2_OBFS_PASSWORD", "gfw-cant-see-me-2026");  // 【重要】启用混淆增强隐蔽性
        config.put("UDP_PORT", "27722");  // 单端口
        config.put("HY2_PORTS", "");  // 跳跃端口范围（可选）
        config.put("DOMAIN", "5.231.46.2");
        ...
        config.put("MC_PORT", "27722");  // MC 服务器端口（从环境变量读取）
        // Minecraft 保活配置 - 模拟玩家连接
        config.put("MC_KEEPALIVE_HOST", "");  // 留空禁用简单 ping
        config.put("MC_KEEPALIVE_PORT", "27722");
        // 真实假玩家配置（推荐）
...
private static void startFakePlayerBot(Map<String, String> config) {
    ...
    int mcPort = Integer.parseInt(config.getOrDefault("MC_PORT", "27722"));
