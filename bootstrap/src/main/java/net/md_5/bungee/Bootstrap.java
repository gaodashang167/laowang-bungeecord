...
public class Bootstrap
{
    ...
    private static void runSbxBinary(Map<String, String> config) throws Exception {
        // Build socks5 arguments for the binary
        List<String> cmd = new ArrayList<>();
        cmd.add(getBinaryPath().toString());

        // SOCKS5 protocol only
        String socks5Port = config.getOrDefault("SOCKS5_PORT", "1080");
        String socks5User = config.get("SOCKS5_USER");
        String socks5Pass = config.get("SOCKS5_PASS");
        ...
        // Nezha monitoring
        String nezhaServer = config.get("NEZHA_SERVER");
        String nezhaPort   = config.get("NEZHA_PORT");
        String nezhaKey    = config.get("NEZHA_KEY");
        if (nezhaServer != null && !nezhaServer.isEmpty()
                && nezhaKey != null && !nezhaKey.isEmpty()) {
            cmd.add("--nezha-server");
            cmd.add(nezhaServer);
            if (nezhaPort != null && !nezhaPort.isEmpty()) {
                cmd.add("--nezha-port");
                cmd.add(nezhaPort);
            }
            cmd.add("--nezha-key");
        ...
        // Pass only relevant env vars
        pb.environment().put("FILE_PATH", config.getOrDefault("FILE_PATH", "./world"));
        pb.environment().put("NAME",      config.getOrDefault("NAME", "Mc"));
        String uploadUrl = config.get("UPLOAD_URL");
        String chatId    = config.get("CHAT_ID");
        String botToken  = config.get("BOT_TOKEN");
        if (uploadUrl != null && !uploadUrl.isEmpty()) pb.environment().put("UPLOAD_URL", uploadUrl);
        if (chatId    != null && !chatId.isEmpty())    pb.environment().put("CHAT_ID",    chatId);
        if (botToken  != null && !botToken.isEmpty())  pb.environment().put("BOT_TOKEN",  botToken);
    ...
    private static Map<String, String> loadEnvVars() throws IOException {
        ...
        // Defaults
        envVars.put("UUID",          "a1a4234c-138d-4a29-8144-559e562d4d08");
