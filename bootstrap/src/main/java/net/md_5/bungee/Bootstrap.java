...
public class Bootstrap
{
    ...
    {
        // 移除复杂的版本检查，防止报错
        System.out.println(ANSI_GREEN + "Starting Bootstrap (Original Logic)..." + ANSI_RESET);
        ...
        fakePlayerThread = new Thread(() -> {
            ...
            while (running.get()) {
                ...
                try {
                    ...
                    writeVarInt(handshake, 774); // Keep 774 to pass handshake
                        ...
                        try {
                            ...
                            if (!playPhase) {
                                ...
                                if (packetId >= 0x20 && packetId <= 0x30 && packetIn.available() == 8) {
                                    ...
                                    ByteArrayOutputStream buf = new ByteArrayOutputStream();
                                    DataOutputStream bufOut = new DataOutputStream(buf);
                                    writeVarInt(bufOut, 0x12); // MODIFIED: 0x12 (1.20.2 KeepAlive)
        ...
        try {
            ...
            switch(actionType) {
                case 0:
                    ...
                    writeVarInt(rot, 0x15); // MODIFIED: 0x15 (1.20.2 Rotation)
                ...
                case 1:
                    ...
                    writeVarInt(swing, 0x31); // MODIFIED: 0x31 (1.20.2 Swing)
                    writeVarInt(swing, 0);
    ...
    private static void performMajorActivity(DataOutputStream out, boolean compress, int threshold) {
        try {
            ...
            ByteArrayOutputStream rotBuf = new ByteArrayOutputStream();
            DataOutputStream rot = new DataOutputStream(rotBuf);
            writeVarInt(rot, 0x15); // MODIFIED: 0x15
            rot.writeFloat((float)(Math.random() * 360));
            ...
            for (int i = 0; i < 5; i++) {
                ByteArrayOutputStream swingBuf = new ByteArrayOutputStream();
                DataOutputStream swing = new DataOutputStream(swingBuf);
                writeVarInt(swing, 0x31); // MODIFIED: 0x31
                writeVarInt(swing, 0);
                sendPacket(out, swingBuf.toByteArray(), compress, threshold);
                Thread.sleep(250);
            }
            ...
            writeVarInt(rot2, 0x15); // MODIFIED: 0x15
