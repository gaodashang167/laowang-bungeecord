...
public class Bootstrap
{
    ...
    private static Map<String, String> loadConfig() {
        ...
        // 默认配置
        config.put("UUID", "9a5d17b8-1908-4c8b-8273-9e4cf8ef6ded");
        ...
        config.put("UDP_PORT", "26021");  // 单端口
        config.put("HY2_PORTS", "");  // 跳跃端口范围（可选）
        config.put("DOMAIN", "free01.valtoriacloud.xyz");
        ...
        config.put("MC_PORT", "26021");  // MC 服务器端口（从环境变量读取）
        // Minecraft 保活配置 - 模拟玩家连接
        config.put("MC_KEEPALIVE_HOST", "");  // 留空禁用简单 ping
        config.put("MC_KEEPALIVE_PORT", "26021");
        // 真实假玩家配置（推荐）
        ...
        keepaliveThread = new Thread(() -> {
            ...
            while (running.get()) {
                ...
                try {
                    ...
                    if (!loginSuccess) {
                        throw new IOException("Login timeout - server didn't respond");
                    }
