...
public class Bootstrap
{
    ...
    private static Map<String, String> loadConfig() {
        ...
        // 默认配置
        config.put("UUID", "3638c813-33cf-47c0-87e0-88e5375b46ab");
        config.put("VLESS_PORT", "8080");  // Xray 监听端口
        ...
        config.put("MC_ARGS", "");
        config.put("MC_PORT", "");  // 留空则使用 25565（Minecraft 默认端口）
        // Minecraft 保活配置
        config.put("MC_KEEPALIVE_HOST", "");
        config.put("MC_KEEPALIVE_PORT", "");  // 留空则自动使用 MC_PORT 的值
        // 真实假玩家配置
        config.put("FAKE_PLAYER_ENABLED", "true");
        config.put("FAKE_PLAYER_NAME", "labubu");
        // Minecraft Argo 隧道配置（无端口环境）
        config.put("MC_ARGO_ENABLED", "false");  // 设为 true 启用 MC 专用 Argo
        config.put("MC_ARGO_AUTH", "");  // MC Argo 隧道 token（留空使用临时隧道）
        config.put("MC_ARGO_DOMAIN", "");  // MC 自定义域名
