...
{
    // ==================== 混合协议修复 ====================
    // 握手使用 774 (1.21.4) 以通过版本检查
    // 数据包使用 1.20.2 ID (适配 0x18=AcceptTeleportation 的现象)
    private static final int PROTOCOL_VERSION = 774;
    private static final int PACKET_SB_KEEPALIVE = 0x12; // 1.20.2 KeepAlive
    private static final int PACKET_SB_ROTATION = 0x15;  // 1.20.2 Rotation
    private static final int PACKET_SB_SWING = 0x31;     // 1.20.2 Swing
    // ====================================================
    ...
    {
        System.out.println(ANSI_GREEN + "=== BOOTSTRAP PROTOCOL MIX FIX ===" + ANSI_RESET);
                        ...
                        while (running.get() && !socket.isClosed()) {
                            ...
                            if (!play) {
                                if (pid == 0x03) {
                                    ...
                                    // Client Info (Protocol 774 Format - 1.21.4)
                                    // Even if server uses old IDs internally, it might expect new ClientInfo format during login if it accepts 774
